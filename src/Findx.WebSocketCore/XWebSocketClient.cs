...
public class XWebSocketClient: IWebSocketSession, IDisposable
{
    ...
    {
        if (Status is XWebSocketState.Open or XWebSocketState.Connecting || _webSocketClient?.State == WebSocketState.Open)
        {
            return;
        }
        ...
        {
            _webSocketClient.Options.SetRequestHeader(kv.Key, kv.Value);
        }
    ...
    public async Task SendAsync(string message, CancellationToken cancellationToken = default)
    {
        ...
        //发送消息
        await _webSocketClient.SendAsync(new ArraySegment<byte>(body), WebSocketMessageType.Text, true, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        //发送消息
        await _webSocketClient.SendAsync(new ArraySegment<byte>(message), WebSocketMessageType.Text, true, cancellationToken).ConfigureAwait(false);
    }
}