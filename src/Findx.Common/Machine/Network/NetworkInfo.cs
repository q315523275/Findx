...
public class NetworkInfo
{
    ...
    /// 224.0.0.1</remarks>
    public IEnumerable<IPAddress> MulticastAddresses => NetworkInterface.GetIPProperties().MulticastAddresses.Select(x => x.Address);

    /// <summary>
    /// 获取分配给此接口的单播地址，ipv4、ipv6
    /// </summary>
    /// <remarks>ex：192.168.3.38</remarks>
    public IEnumerable<IPAddress> UnicastAddresses => NetworkInterface.GetIPProperties().UnicastAddresses.Select(x => x.Address);

    /// <summary>
    /// 获取此接口的 IPv4 网关地址，ipv4、ipv6
    /// </summary>
    /// <remarks>ex：fe80::1677:40ff:fef9:bf95%5、192.168.3.1</remarks>
    public IEnumerable<IPAddress> GatewayAddresses => NetworkInterface.GetIPProperties().GatewayAddresses.Select(x => x.Address);
    ...
    public static IPAddress TryGetRealIpv4()
    {
        if (Common.IsLinux)
        {
            return NetworkInterface.GetAllNetworkInterfaces().Select(p => p.GetIPProperties()).SelectMany(p => p.UnicastAddresses)
    ...
    public static NetworkInfo TryGetRealNetworkInfo()
    {
        ...
        return GetNetworkInfos().FirstOrDefault(x => x.UnicastAddresses.Any(address => address.MapToIPv4().ToString() == realIp.MapToIPv4().ToString()));
    }
