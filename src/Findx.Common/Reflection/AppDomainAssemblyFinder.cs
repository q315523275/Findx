...
public class AppDomainAssemblyFinder : FinderBase<Assembly>, IAppDomainAssemblyFinder
{
    ...
    private readonly string[] _filter =
        { "System", "Microsoft", "netstandard", "dotnet", "Window", "mscorlib", "Newtonsoft", "Remotion.Linq" };
    ...
    {
        foreach (var f in files) yield return Assembly.Load(new AssemblyName(f));
    }
    ...
    protected override IEnumerable<Assembly> FindAllItems()
    {
        ...
        if (context != null)
        {
            var lt = context.GetDefaultAssemblyNames()
                .Where(x => x.Name != null &&
                            !_filter.Any(m => x.Name.StartsWith(m, StringComparison.OrdinalIgnoreCase)))
        ...
        return files
            .WhereIf(_filterSystemAssembly,
                name => !_filter.Any(n => name.StartsWith(n, StringComparison.OrdinalIgnoreCase))).Distinct()
