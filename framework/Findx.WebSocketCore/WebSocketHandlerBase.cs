...
public abstract class WebSocketHandlerBase
{
    ...
    public async Task SendMessageAsync<T>(WebSocketClient client, WebSocketMessage<T> message)
    {
        ...
        try
        {
            ...
            await client.Client.SendAsync(body, WebSocketMessageType.Text, true, CancellationToken.None)
                .ConfigureAwait(false);
        }
        catch (WebSocketException e)
        {
            if (e.WebSocketErrorCode == WebSocketError.ConnectionClosedPrematurely) await OnDisconnected(client);
        }
    }
