...
public class DirectoryTool
{
    ...
    public static bool Delete(string directory, bool isDeleteRoot = true)
    {
        ...
        if (!dirPathInfo.Exists)
            return false;
        // 删除目录下所有文件
        foreach (var fileInfo in dirPathInfo.GetFiles()) fileInfo.Delete();
        // 递归删除所有子目录
        foreach (var subDirectory in dirPathInfo.GetDirectories()) Delete(subDirectory.FullName);
        // 删除目录
        if (isDeleteRoot)
        {
    ...
    public static void Copy(string sourcePath, string targetPath, string[] searchPatterns = null)
    {
        ...
        if (!Directory.Exists(sourcePath)) throw new DirectoryNotFoundException("递归复制文件夹时源目录不存在。");
    ...
    {
        if (!Directory.Exists(sourceDirName)) throw new DirectoryNotFoundException("转移文件夹时源目录不存在。");
    ...
    /// <returns></returns>
    public static IEnumerable<string> EnumerateFiles(string directoryPath, string searchPattern,
        bool topDirectoryOnly = true)
    {
        if (!Directory.Exists(directoryPath)) throw new DirectoryNotFoundException("源目录不存在。");
